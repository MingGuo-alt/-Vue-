<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>销售代表管理</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card style="margin-top: 10px">
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input placeholder="请输入名称" v-model="inputContent" clearable>
            <el-button
              slot="append"
              icon="el-icon-search"
              @click="inquiry"
      ...
      <el-table :data="userlist" border stripe class="user_message_table">
        ...
        <el-table-column label="所属部门" prop="depart_name"> </el-table-column>
        <el-table-column
          label="所属战区"
          prop="war_zone_name"
        ></el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="所属部门ID" prop="sales_depart">
          <el-input v-model="addForm.sales_depart"></el-input>
        </el-form-item>
    ...
    >
      ...
      >
        ...
        <el-form-item label="所属部门" prop="sales_depart">
          <el-input v-model="editForm.sales_depart"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    async getUserList() {
      this.isQuery = false;
      const { data: res } = await this.$http.get("/users/sales/", {
        params: {
          // sales_name: window.sessionStorage.getItem("user_id"),
      ...
      for (var j = 0; j < res.results.length; j++) {
        this.userlist[j]["depart_name"] = res.results[j].sales_depart.name;
        this.userlist[j]["war_zone_name"] = res.results[j].war_zone.name;
      }
      //   console.log(res)

      //   console.log(res.results.sales_depart.name)
      //   this.userlist["deaprt_name"] = res.results.sales_depart.name
    ...
    editUserInfo() {
      this.$refs.editFormRef.validate(async (valid) => {
        ...
        // 发起修改用户信息的数据请求
        const { data: res } = await this.$http.post(
          "/users/sales/update/?id=" + this.updateid,
          this.editForm
        );
        // 关闭对话框
    ...
    async removeUserById(id) {
      ...
      // console.log('确认了删除')
      const { data: res } = await this.$http.delete("users/sales/" + id + "/");
